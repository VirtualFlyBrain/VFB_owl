...
progress_chat "$job";
java -Xmx6000m -Xss515m -cp $CP"*" org.python.util.jython fb_feat_ont_gen.py $KB_old_endpoint $USR $PD
exit_on_fail $? "$job"
...
#   java -Xmx6000m -Xss515m -cp $CP"*" org.python.util.jython vfb_ind_runner.py $KB_endpoint $USR $PD $var $FBBT
    sbt \"run -f --usr=$USR --pwd=$PWD --endpoint=$ENDPOINT --dataset=$var  --ontology=$FBBT
...
#sbt \"run -f --usr=$USR --pwd=$PWD --endpoint=$ENDPOINT --dataset=""  --ontology=$FBBT
#exit_on_fail $? $job
